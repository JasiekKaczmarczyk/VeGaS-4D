...
from nerfstudio.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation_from_rotor8, strip_symmetric, \
    build_scaling_rotation, quaterion2rotor, rotornorm, slice_4d, slice_4d_vegas
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        weights = torch.nn.Parameter(torch.rand((xyz.shape[0], 3 * self.config.poly_degree), device="cuda") * 0.001 - 0.0005, requires_grad=True)
    ...
    def load_state_dict(self, dict, **kwargs):  # type: ignore
        ...
        if "xyz" in dict:
            ...
            for p in ["xyz","t", "scales_xyz","scales_t", "weights" ,"quats1","quats2", "features_dc", "features_rest", "opacities"]:
    ...
    def split_gaussians(self, split_mask,samps):
        ...
        new_weights = self.weights[split_mask].repeat(samps, 1)
    ...
    def get_gaussian_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        return {
            ...
            for name in ["xyz", "t","scales_xyz","scales_t", "weights", "quats1","quats2", "features_dc", "features_rest", "opacities"]
        }
    ...
    def forward_1(self,camera,background):
        ...
        render =  {
            ...
            # "speed_masked": speed[spatial_mask],
    ...
    def forward_2(self,camera,background):
        # means4d, conv3, speed, w = self.slice_4d_to_3d(t_current=camera[1].times[0][0])
        ...
        for cam in camera:
            ...
            means3d, conv3, speed, w = self.slice_4d_to_3d(t_current=cam.times[0][0])

            opacity_aftersigmopid = self.get_opacities
            delta_t = self.xyzt[:, 3] - cam.times[0][0]
        ...
        # self.render["speed_full"] = self.weights[:, :3]
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if True:
            ...
            # points4d, speed = self.render["mean4d_full"], self.weights
            points4d, speed = self.render["mean4d_full"], self.render["speed_full"]
            mask_t = self.render['mask_t']
            points4d_msked, speed_masked = points4d[mask_t], speed[mask_t]
            ...
            knn_speed4d_pxkx3 = speed_masked[knnre4d].reshape(-1, K_knn, 3*self.config.poly_degree)
            knn_mask_pxkx1 = knnmask_pxk.unsqueeze(-1)
            knn_validnum_px1 = knnmask_pxk.sum(dim=-1, keepdim=True)
            knn_mask_p  = (knn_validnum_px1[:, 0] > 0).float()
        ...
        if True:
            ...
            loss_weight_reg = torch.norm(self.weights, p=2, dim=-1).mean()
            self.weightloss = 0.01 * loss_weight_reg
    ...
    def slice_4d_to_3d(self, t_current=None, mask=None):
        ...
        # scaling_factor = norm_gauss(xyzt[:, 3], self.scales[:, 3], t_current)
        # scale[:, :3] = scale[:, :3] * scaling_factor[:, None]
        # scale3d = self.scales[:, :3]
        # scale_t = self.scales[:, 3:4]
        # scale3d = scale3d * scaling_factor[:, None]

        # scale = torch.cat([scale3d, scale_t], dim=-1)
        ...
        if mask is not None:
            ...
            r1 = r1[mask]
            r2 = r2[mask]
        # conv3, speed, w = slice_4d_vegas(scale, r1, r2, scaling_factor)
        conv3, speed, w = slice_4d(scale, r1, r2)
        if t_current is not None:
            deltat = t_current - xyzt[:, 3:4]

            speed_full = self.weights.clone()
            speed_full[:, :3] = self.weights[:, :3] + speed

            # mean3d = xyzt[:, :3] + speed * deltat
            mean3d = xyzt[:, :3]
            poly_weights = torch.chunk(speed_full, chunks=self.config.poly_degree, dim=-1)
            for i, poly_weight in enumerate(poly_weights):
                mean3d = mean3d + poly_weight * (deltat ** (i+1))
        else:
            mean3d = None
        return mean3d, conv3, speed_full, w 
    def slice_4d_to_3d_without_T(self, scaling_factor,mask=None):
        xyzt = self.xyzt
        scale = self.scales
        r1, r2 = self.quats1,self.quats2
        if mask is not None:
    ...
    def slice_4d_to_3d_with_T(self, xyzt, speed, t_current):
        ...
        mean3d = xyzt[:, :3]
        poly_weights = torch.chunk(self.weights, chunks=self.config.poly_degree, dim=-1)
        for i, poly_weight in enumerate(poly_weights):
            mean3d = mean3d + poly_weight * (deltat ** (i+1))
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        return {
            "main_loss": (1 - self.config.ssim_lambda) * Ll1 + self.config.ssim_lambda * (1-simloss)+self.entropyloss+self.knnloss+self.weightloss,
            'scale_reg':scale_reg
        }
